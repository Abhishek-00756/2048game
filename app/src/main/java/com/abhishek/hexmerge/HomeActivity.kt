...
class HomeActivity: ComponentActivity(){
 ...
 private fun home(){
  val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;gravity=Gravity.CENTER;setPadding(24,24,24,24);background=GradientDrawable(GradientDrawable.Orientation.TL_BR,intArrayOf(Color.rgb(15,42,80),Color.rgb(5,17,35)))}
  root.addView(TextView(this).apply{text="HEX\nMERGE";textSize=42f;gravity=Gravity.CENTER;setTextColor(Color.WHITE);typeface=Typeface.DEFAULT_BOLD},LinearLayout.LayoutParams(-1,0,1f))
  root.addView(TextView(this).apply{text="LEVEL ${engine.level}   •   ★ ${engine.coins}\nBEST ${engine.bestScore}";textSize=16f;gravity=Gravity.CENTER;setTextColor(Color.LTGRAY)},LinearLayout.LayoutParams(-1,80))
  button(root,"PLAY"){startActivity(Intent(this,MainActivity::class.java))};button(root,"LEVELS"){levels()};button(root,"DAILY CHALLENGE"){daily()};button(root,"SHOP"){shop()};button(root,"SETTINGS"){settings()}
 ...
 private fun levels(){val items=(1..20).map{n->"Level $n  • Target ${32 shl (n-1).coerceAtMost(14)}  ${"★".repeat(engine.stars[n]?:0)}"}.toTypedArray();AlertDialog.Builder(this).setTitle("LEVEL SELECT").setItems(items){_,i->if(i+1<=engine.unlockedLevel){engine.reset(i+1);save.save(engine);startActivity(Intent(this,MainActivity::class.java))}else Toast.makeText(this,"Complete earlier levels to unlock",Toast.LENGTH_SHORT).show()}.setNegativeButton("CLOSE",null).show()}
 private fun daily(){AlertDialog.Builder(this).setTitle("DAILY CHALLENGE").setMessage("Target: 512\nToday's best: ${DailyChallengeManager(this).best()}\n\nThe daily seed is deterministic for the current day.").setPositiveButton("PLAY"){_,_->engine.reset(1);save.save(engine);startActivity(Intent(this,MainActivity::class.java))}.setNegativeButton("CLOSE",null).show()}
